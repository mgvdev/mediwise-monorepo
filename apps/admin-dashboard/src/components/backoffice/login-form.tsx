...
export function BackofficeLoginForm() {
	...
	const sendOtp = useCallback(async () => {
		...
		if (!trimmedEmail) {
			toast.error("Please enter your Mediwise email.");
			return;
		}
	...
	if (isPending) {
		return (
			<Card className="border-border/60 bg-card/80 w-full max-w-md rounded-3xl border p-6 shadow-lg">
		...
		<Card className="border-border/60 bg-card/80 w-full max-w-md rounded-3xl border p-6 shadow-lg">
			<Card.Header className="flex flex-col items-start gap-2">
				...
				<p className="text-muted-foreground text-sm">
					Sign in with your Mediwise email to receive a one-time code.
				</p>
			...
			<Card.Content className="mt-4">
				<form className="space-y-4" onSubmit={handleSubmit}>
					<div className="space-y-2">
						<Label htmlFor="login-email">Mediwise email</Label>
						<Input
							id="login-email"
							type="email"
							fullWidth
